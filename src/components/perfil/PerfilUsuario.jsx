...
export default function PerfilUsuario({ rol, nombre, correo, descripcion, datos, completarRuta }) {
    return (
        <section className="max-w-5xl mx-auto">
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="bg-[#071D2B] px-6 py-8 sm:px-8 flex flex-col sm:flex-row sm:items-center gap-5">
                    ...
                    <div>
                        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#8AFD5D]">{rol}</p>
                        <h2 className="mt-1 text-2xl font-black text-white">{nombre}</h2>
                        <p className="mt-1 text-sm text-slate-300">{correo}</p>
                    </div>
                </div>
                ...
                <div className="p-6 sm:p-8">
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
                        <div>
                            <h1 className="text-xl font-bold text-[#071D2B]">Mi perfil</h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {datos.map(({ etiqueta, valor, icono }) => (
                            <div key={etiqueta} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                <div className="flex items-center gap-2 text-slate-400">
                                    <i className={`bi ${icono}`} />
                                    <p className="text-xs font-bold uppercase tracking-wide">{etiqueta}</p>
                                </div>
                                <p className="mt-2 text-sm font-semibold text-slate-700">{valor}</p>
                            </div>
                        ))}
                    </div>
                </div>
